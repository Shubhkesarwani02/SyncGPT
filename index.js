...
client.on('messageCreate', async (message) => {
    ...
    if (fullInput.length > maxLength) {
        message.reply(`Your question is too long. It will be shortened to ${maxLength} characters.`);
    }
    ...
    try {
        ...
        const reply = response.data?.candidates?.[0]?.content?.parts?.[0]?.text || "Gemini did not return a reply.";
